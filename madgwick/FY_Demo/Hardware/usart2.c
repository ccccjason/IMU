...
**
**
**Description:DMA的USART通道5为RX，通道4为TX
*/
...
//串口发送缓存区
u8 USART2_TX_BUF[USART2_MAX_SEND_LEN] = {"feiyutech"};  //发送缓冲,最大USART2_MAX_SEND_LEN字节
...
void USART2_printf(char* fmt, ...)
{
    ...
    UART_DMA_Enable(DMA1_Channel4,
                    strlen((const char*)USART2_TX_BUF)); //通过dma发送出去
}
...
void TIM3_Init(u16 arr, u16 psc) //1000,48,48000000/48/1000=1000HZ=1ms中断
{
    NVIC_InitTypeDef NVIC_InitStructure;
...
void UART_DMA_Config(DMA_Channel_TypeDef* DMA_CHx, u32 cpar, u32 cmar)
{
    ...
    DMA_InitStructure.DMA_DIR =
        DMA_DIR_PeripheralDST;  //数据传输方向，从内存读取发送到外设
