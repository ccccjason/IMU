...
void NMEA_GPRMC_Analysis(nmea_msg* gpsx, u8* buf)
{
    ...
    u32 temp;
    float rs;
    p1 = (u8*)strstr((const char*)buf,
                     "GPRMC"); //"$GPRMC",经常有&和GPRMC分开的情况,故只判断GPRMC.
    ...
    if (posx != 0XFF) {
        ...
        gpsx->latitude = gpsx->latitude * NMEA_Pow(10, 5) + (rs * NMEA_Pow(10,
                         5 - dx)) / 60; //转换为°
    }
    ...
    if (posx != 0XFF) {
        ...
        rs = temp % NMEA_Pow(10, dx + 2);       //得到'
        gpsx->longitude = gpsx->longitude * NMEA_Pow(10, 5) + (rs * NMEA_Pow(10,
                          5 - dx)) / 60; //转换为°
    }
...
u8 Ublox_Cfg_Ack_Check(void)
{
    ...
    if (len < 250) { //超时错误.
        ...
        for (i = 0; i < len; i++)if (USART2_RX_BUF[i] == 0XB5) {
                break;    //查找同步字符 0XB5
            }

        if (i == len) {
            rval = 2;    //没有找到同步字符
        } else if (USART2_RX_BUF[i + 3] == 0X00) {
            rval = 3;    //接收到NACK应答
        } else {
            rval = 0;    //接收到ACK应答
        }
...
u8 Ublox_Cfg_Cfg_Save(void)
{
    ...
    while (DMA1_Channel5->CNDTR != 0); //等待通道7传输完成

    UART_DMA_Enable(DMA1_Channel5, sizeof(_ublox_cfg_cfg)); //通过dma发送出去

    for (i = 0; i < 6; i++)if (Ublox_Cfg_Ack_Check() == 0) {
            break;    //EEPROM写入需要比较久时间,所以连续判断多次
        }
...
u8 Ublox_Cfg_Msg(u8 msgid, u8 uart1set)
{
    ...
    UART_DMA_Enable(DMA1_Channel5, sizeof(_ublox_cfg_msg)); //通过dma发送出去
...
u8 Ublox_Cfg_Prt(u32 baudrate)
{
    ...
    UART_DMA_Enable(DMA1_Channel5, sizeof(_ublox_cfg_prt)); //通过dma发送出去
    delay_ms(200);              //等待发送完成
...
u8 Ublox_Cfg_Tp(u32 interval, u32 length, signed char status)
{
    ...
    UART_DMA_Enable(DMA1_Channel5, sizeof(_ublox_cfg_tp));  //通过dma发送出去
    return Ublox_Cfg_Ack_Check();
}
//配置UBLOX NEO-6的更新速率
...
u8 Ublox_Cfg_Rate(u16 measrate, u8 reftime)
{
    ...
    Ublox_CheckSum((u8*)(&cfg_rate->id), sizeof(_ublox_cfg_rate) - 4,
                   &cfg_rate->cka, &cfg_rate->ckb);

    while (DMA1_Channel5->CNDTR != 0); //等待通道7传输完成

    UART_DMA_Enable(DMA1_Channel5, sizeof(_ublox_cfg_rate)); //通过dma发送出去
    return Ublox_Cfg_Ack_Check();
}
