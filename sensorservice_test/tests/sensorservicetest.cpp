...
#if 1
                printf("accelerometer %8f\t%8f\t%8f\n",
                        buffer[i].data[0], buffer[i].data[1], buffer[i].data[2]);
                        
#else
                printf("accelerometer %lld\t%8f\t%8f\t%8f\t%f\n",
                        buffer[i].timestamp,
                        buffer[i].data[0], buffer[i].data[1], buffer[i].data[2],
                        1.0/t);
#endif
...
int main(int argc, char** argv)
{
    ...
    Sensor const* gyro = mgr.getDefaultSensor(Sensor::TYPE_GYROSCOPE);

    printf("accelerometer=%p (%s)\n", accelerometer, accelerometer->getName().string());
    printf("gyro=%p (%s)\n", gyro , gyro->getName().string());
