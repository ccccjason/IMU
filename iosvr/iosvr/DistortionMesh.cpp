                               ...
                               bool vignetteEnabled) : indices(-1), arrayBufferID(-1), elementBufferID(-1)
{
    ...
    for (int row = 0; row < rows; ++row) {
        for (int col = 0; col < cols; ++col) {
            const GLfloat uTextureBlue = col / 39.0f * (viewportWidthTexture / textureWidth)
                                         + viewportXTexture / textureWidth;
            const GLfloat vTextureBlue = row / 39.0f * (viewportHeightTexture /
            ...
            const GLfloat textureToScreenBlue = (rTexture > 0.0f) ?
                                                distortionBlue->distortInverse(rTexture) / rTexture : 1.0f;
            ...
            const GLfloat screenToTextureGreen = (rScreen > 0.0f) ?
                                                 distortionGreen->getDistortionFactor(rScreen) : 1.0f;
            const GLfloat uTextureGreen = (xScreen * screenToTextureGreen +
                                           xEyeOffsetTexture) / textureWidth;
            const GLfloat vTextureGreen = (yScreen * screenToTextureGreen +
                                           yEyeOffsetTexture) / textureHeight;

            const GLfloat screenToTextureRed = (rScreen > 0.0f) ?
                                               distortionRed->getDistortionFactor(rScreen) : 1.0f;
            const GLfloat uTextureRed = (xScreen * screenToTextureRed + xEyeOffsetTexture) /
                                        textureWidth;
            const GLfloat vTextureRed = (yScreen * screenToTextureRed + yEyeOffsetTexture) /
                                        textureHeight;

            const GLfloat vignetteSizeTexture = vignetteSizeTanAngle / textureToScreenBlue;

            const GLfloat dxTexture = xTexture + xEyeOffsetTexture - hfgl::clampf(
                                          xTexture + xEyeOffsetTexture,
                                          viewportXTexture + vignetteSizeTexture,
                                          viewportXTexture + viewportWidthTexture - vignetteSizeTexture);
            const GLfloat dyTexture = yTexture + yEyeOffsetTexture - hfgl::clampf(
