...
{
    if (mGlStateBackup != 0) {
        delete mGlStateBackup;
    }

    if (mGlStateBackupAberration != 0) {
        delete mGlStateBackupAberration;
    }

    if (mLeftEyeDistortionMesh != 0) {
        delete mLeftEyeDistortionMesh;
    }

    if (mRightEyeDistortionMesh != 0) {
        delete mRightEyeDistortionMesh;
    }

    if (mLeftEyeViewport != 0) {
        delete mLeftEyeViewport;
    }

    if (mRightEyeViewport != 0) {
        delete mRightEyeViewport;
    }

    if (mProgramHolder != 0) {
        delete mProgramHolder;
    }

    if (mProgramHolderAberration != 0) {
        delete mProgramHolderAberration;
    }
}

void DistortionRenderer::setTextureFormat(GLint _textureFormat,
        GLint _textureType)
{
    if (mDrawingFrame) {
        std::cerr << "Cannot change texture format during rendering" << std::endl;
    } else if (this->mTextureFormat != _textureFormat ||
...
void DistortionRenderer::undistortTexture(GLint _textureID)
{
    ...
    glScissor(screen->getWidth() / 2, 0, screen->getWidth() / 2,
              screen->getHeight());
                                        ...
                                        GLfloat _virtualEyeToScreenDistance)
{
    ...
    mXPxPerTanAngle = screen->getWidth() / (screen->getWidthInMeters() /
                                            mMetersPerTanAngle);
    mYPxPerTanAngle = screen->getHeight() / (screen->getHeightInMeters() /
                      mMetersPerTanAngle);
    mFovsChanged = true;
    mViewportsChanged = true;
}
...
void DistortionRenderer::updateViewports(Viewport* _leftViewport,
        Viewport* _rightViewport)
{

    _leftViewport->setViewport(roundf(mLeftEyeViewport->x * mXPxPerTanAngle *
                                      mResolutionScale),
    ...
    _rightViewport->setViewport(roundf(mRightEyeViewport->x * mXPxPerTanAngle *
                                       mResolutionScale),
...
void DistortionRenderer::updateTextureAndDistortionMesh()
{
    ...
    CardboardDeviceParams* cardboardDeviceParams =
        mHeadMountedDisplay->getCardboard();
    ...
    GLfloat textureWidthTanAngle = mLeftEyeViewport->width +
                                   mRightEyeViewport->width;
    GLfloat textureHeightTanAngle = std::max(mLeftEyeViewport->height,
                                    mRightEyeViewport->height);
    GLint maxTextureSize = 0;

    glGetIntegerv(GL_MAX_TEXTURE_SIZE, &maxTextureSize);
    GLint textureWidthPx = std::min<float>(round(textureWidthTanAngle *
                                           mXPxPerTanAngle), maxTextureSize);
    GLint textureHeightPx = std::min<float>(round(textureHeightTanAngle *
                                            mYPxPerTanAngle), maxTextureSize);

    GLfloat xEyeOffsetTanAngleScreen =
        (screen->getWidthInMeters() / 2.0f -
         cardboardDeviceParams->getInterLensDistance() / 2.0f) / mMetersPerTanAngle;
    GLfloat yEyeOffsetTanAngleScreen =
        (cardboardDeviceParams->getVerticalDistanceToLensCenter() -
         screen->getBorderSizeInMeters()) / mMetersPerTanAngle;
    ...
    mLeftEyeDistortionMesh = createDistortionMesh(mLeftEyeViewport,
                             textureWidthTanAngle, textureHeightTanAngle, xEyeOffsetTanAngleScreen,
                             yEyeOffsetTanAngleScreen);

    xEyeOffsetTanAngleScreen = screen->getWidthInMeters() / mMetersPerTanAngle -
...
DistortionRenderer::EyeViewport* DistortionRenderer::initViewportForEye(
    FieldOfView* _eyeFieldOfView, GLfloat _xOffset)
{
...
DistortionMesh* DistortionRenderer::createDistortionMesh(
    EyeViewport* _eyeViewport,
...
void DistortionRenderer::renderDistortionMesh(DistortionMesh* _mesh,
        GLint _textureID)
{
    ...
    glVertexAttribPointer(_programHolder->positionLocation, 2, GL_FLOAT, GL_FALSE,
                          9 * sizeof(GLfloat), (void*)(0 * sizeof(GLfloat)));
    glEnableVertexAttribArray(_programHolder->positionLocation);
    glVertexAttribPointer(_programHolder->vignetteLocation, 1, GL_FLOAT, GL_FALSE,
                          9 * sizeof(GLfloat), (void*)(2 * sizeof(GLfloat)));
    glEnableVertexAttribArray(_programHolder->vignetteLocation);
    glVertexAttribPointer(_programHolder->blueTextureCoordLocation, 2, GL_FLOAT,
                          GL_FALSE, 9 * sizeof(GLfloat), (void*)(7 * sizeof(GLfloat)));
    glEnableVertexAttribArray(_programHolder->blueTextureCoordLocation);

    if (mChromaticAberrationCorrectionEnabled) {
        glVertexAttribPointer(_programHolder->redTextureCoordLocation, 2, GL_FLOAT,
                              GL_FALSE, 9 * sizeof(GLfloat), (void*)(3 * sizeof(GLfloat)));
        glEnableVertexAttribArray(_programHolder->redTextureCoordLocation);
        glVertexAttribPointer(_programHolder->greenTextureCoordLocation, 2, GL_FLOAT,
                              GL_FALSE, 9 * sizeof(GLfloat), (void*)(5 * sizeof(GLfloat)));
...
GLfloat DistortionRenderer::computeDistortionScale(Distortion* _distortion,
        GLfloat _screenWidthM, GLfloat _interpupillaryDistanceM)
{
    return _distortion->getDistortionFactor((_screenWidthM / 2.0f -
                                            _interpupillaryDistanceM / 2.0f) / (_screenWidthM / 4.0f));
}

GLuint DistortionRenderer::createTexture(GLint _width, GLint _height,
        GLint _textureFormat, GLint _textureType)
{
    GLuint _textureID = 0;
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexImage2D(GL_TEXTURE_2D, 0, _textureFormat, _width, _height, 0,
                 _textureFormat, _textureType, 0);
...
GLuint DistortionRenderer::setupRenderTextureAndRenderbuffer(GLint _width,
        GLint _height)
{
    if (mTextureID != -1) {
    ...
    if (status != GL_FRAMEBUFFER_COMPLETE) {
        std::cerr << "DistortionRenderer: Framebuffer is not complete " << status <<
                  std::endl;
        return 0;
    }
...
GLuint DistortionRenderer::createProgram(const GLchar* vertexSource,
...
DistortionProgramHolder* DistortionRenderer::createProgramHolder(
    bool aberrationCorrected)
{
    ...
    if (aberrationCorrected) {
        holder->program = createProgram(vertexShaderAberration,
                                        fragmentShaderAberration);
    ...
    if (holder->positionLocation == -1) {
        std::cerr << "DistortionRenderer Could not get attrib location for aPosition" <<
                  std::endl;
        delete holder;
        return 0;
    }
    ...
    if (holder->vignetteLocation == -1) {
        std::cerr << "DistortionRenderer Could not get attrib location for aVignette" <<
                  std::endl;
        delete holder;
        return 0;
    }
    ...
    if (aberrationCorrected) {
        holder->redTextureCoordLocation = glGetAttribLocation(holder->program,
                                          "aRedTextureCoord");
#if defined(DEBUG)
        hfgl::CheckGlErrors();
#endif
        ...
        if (holder->redTextureCoordLocation == -1) {
            std::cerr <<
                      "DistortionRenderer Could not get attrib location for aRedTextureCoord" <<
                      std::endl;
            delete holder;
            return 0;
        }
        ...
        holder->greenTextureCoordLocation = glGetAttribLocation(holder->program,
                                            "aGreenTextureCoord");
#if defined(DEBUG)
        hfgl::CheckGlErrors();
#endif
        ...
        if (holder->greenTextureCoordLocation == -1) {
            std::cerr <<
                      "DistortionRenderer Could not get attrib location for aGreenTextureCoord" <<
                      std::endl;
            delete holder;
            return 0;
        }
    ...
    holder->blueTextureCoordLocation = glGetAttribLocation(holder->program,
                                       "aBlueTextureCoord");
#if defined(DEBUG)
    hfgl::CheckGlErrors();
#endif
    ...
    if (holder->blueTextureCoordLocation == -1) {
        std::cerr <<
                  "DistortionRenderer Could not get attrib location for aBlueTextureCoord" <<
                  std::endl;
        delete holder;
        return 0;
    }
    ...
    holder->uTextureCoordScaleLocation = glGetUniformLocation(holder->program,
                                         "uTextureCoordScale");
#if defined(DEBUG)
    hfgl::CheckGlErrors();
#endif
    ...
    if (holder->uTextureCoordScaleLocation == -1) {
        std::cerr <<
                  "DistortionRenderer Could not get attrib location for uTextureCoordScale" <<
                  std::endl;
        delete holder;
        return 0;
    }

    holder->uTextureSamplerLocation = glGetUniformLocation(holder->program,
                                      "uTextureSampler");
#if defined(DEBUG)
    hfgl::CheckGlErrors();
#endif
